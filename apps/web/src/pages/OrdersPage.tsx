...
function DispatchModal({ order, onClose }: { order: Order; onClose: () => void }) {
  const queryClient = useQueryClient()
  ...
  // Pre-fill quantities with remaining amounts
  useEffect(() => {
    const initial: Record<string, string> = {}
    for (const item of order.order_items) {
      const rem = remainingKg(item)
      if (rem > 0) initial[item.id] = String(Math.round(rem))
    }
  ...
  const handleSubmit = async () => {
    ...
    if (lines.length === 0) { setError('Enter at least one quantity to dispatch.'); return }

    for (const { item, qty } of lines) {
      if (qty > remainingKg(item)) {
        setError(`${item.lots?.name ?? 'Item'}: cannot dispatch more than remaining (${Math.round(remainingKg(item))} kg).`)
        return
      }
    }
    ...
    // Insert dispatch items + FIFO batch deduction
    for (const { item, qty } of lines) {
      ...
      // FIFO: deduct from oldest batches of this lot
      let remaining = qty
    ...
    // Compute from in-memory data (avoids timing issues with nested join post-insert)
    const fullyDispatched = order.order_items.every(item => {
      const qty = parseFloat(quantities[item.id] || '0')
      return remainingKg(item) - qty <= 0
    })
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <Card className="w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <CardHeader>
          <CardTitle className="text-base">Dispatch — {order.os_number}</CardTitle>
          <p className="text-xs text-gray-500 mt-0.5">{order.clients?.company_name}</p>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>DR# *</Label>
              <Input value={drNumber} onChange={e => setDrNumber(e.target.value)} placeholder="DR-001" />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <Label>Received By <span className="text-gray-400 font-normal text-xs">optional</span></Label>
            <Input value={receiverName} onChange={e => setReceiverName(e.target.value)} placeholder="Name of receiver" />
          </div>

          {/* Line items */}
          <div>
            <p className="text-xs font-medium text-gray-500 mb-2">Quantities to Dispatch</p>
            <div className="space-y-2">
              {itemsToDispatch.map(item => {
                const ordered = parseFloat(item.weight_ordered_kg)
                const already = dispatchedKg(item)
                const remaining = remainingKg(item)
                return (
                  <div key={item.id} className="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{item.lots?.name ?? '—'}</p>
                      <p className="text-xs text-gray-400">
                        Ordered {Math.round(ordered)} kg
                        {already > 0 && ` · ${Math.round(already)} kg already dispatched`}
                    ...
                    <span className="text-xs text-gray-400 w-4">kg</span>
                  </div>
                )
              })}
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any dispatch notes…" />
          </div>
